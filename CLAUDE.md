# Synq — Daily Planner (AI Agent Guide)

Tauri v2 desktop app with an embedded HTTP server. This file documents how to run headless, authenticate, and drive the app entirely via REST API.

## Build

```bash
# From project root — builds frontend then Rust binary
...
**macOS:** Run the compiled binary directly (no window server needed for the HTTP server):
```bash
./src-tauri/target/release/daily-planner &
```

**Linux:** Unset display env vars so Tauri skips the window:
```bash
...
The SQLite database is at the Tauri app data directory (`com.synq.app`):
...
The DB uses WAL mode with foreign keys enabled.

## Auth Token

All `/api/*` routes except `GET /api/health` require authentication. The token is a 32-char alphanumeric string auto-generated on first launch.
...
Set a predictable token before launching (so tests don't need to query the DB):
...
The React SPA is served at the root. Access it in a browser with token auth:
